...
namespace AudioSystem
{
    public class AudioManager : MonoBehaviour
    {
        ...
        public void Init(AudioPool pool, AudioMixerGroup musicMixer, AudioMixerGroup sfxMixer,
        ...
        void Awake()
        {
            ...
            if (!_pool) _pool = GetComponent<AudioPool>();
        ...
        public void PlayMusic(AudioData musicData, float vol = 1, float transitionTime = 1)
        {
        ...
        public void PlaySound(AudioData audio)
        {
            if (!audio) return;
        ...
        public void PlaySound(AudioData audio, float delay)
        {
            if (!audio) return;
            StartCoroutine(PlaySoundDelayed(audio, delay));
        }

        IEnumerator PlaySoundDelayed(AudioData audio, float delay)
        {
            yield return new WaitForSeconds(delay);
            PlaySound(audio);
        }

        public void PlaySound3D(AudioData audio, Vector3 soundPosition)
        {
            if (!audio) return;
            var src = _pool.GetSource();
            src.spatialBlend = 1;
